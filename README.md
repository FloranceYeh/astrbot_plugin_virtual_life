...
- 日程主动窗口默认在计划时间前后 `15` 分钟内确定性随机偏移，且不会超出对应活动时段。
...
启用 `reply_delay_settings` 后，普通私聊和群聊会按第一条消息到达时生效的日程项计算回复延迟。命令、主动消息、日程生成以及其他插件显式构造的 LLM 请求不参与延迟。当天没有内存中的有效日程时直接放行，避免为了计算延迟而提前请求日程生成 LLM。
