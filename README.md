...
管理员可在目标会话直接使用 `/虚拟人生 订阅会话`，插件会自动识别私聊或群聊、启用对应主动消息类型并将当前 UMO 写入白名单。也可使用 `/sid` 获取完整 UMO 后在配置中手动维护。日程查询和普通聊天状态注入不要求会话进入主动消息白名单。
...
- 主动消息分段默认使用关键词模式，原文不超过 `150` 字符时按中英文句末标点与换行拆分；超过阈值时整条发送
- 分段模式可切换为 `regex`；支持内容清理正则，以及 `log` 或 `random` 两种发送间隔，默认使用 `log`
- 回访任务重启补发窗口：`30` 分钟
