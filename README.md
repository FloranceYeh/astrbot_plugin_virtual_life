...
- `/主动消息 状态`
- `/主动消息 立即`（管理员）
- `/主动消息 执行时间`：列出当前会话全部日程窗口、延迟窗口、沉默主动、睡眠异常和用户回访的具体执行时间
- `/主动消息 回访列表`
- `/主动消息 取消回访 <任务ID>`
...
> **单次请求估算（样本）**：普通聊天的智能注入会增加约 `0-510` Token 上下文；完整日程首轮生成约 `1,600-2,300` Token；局部重写约 `1,050-1,700` Token；每次大时间表生成约 `900-1,350` Token。

| 操作 | 单次请求估算区间 | 说明 |
| --- | ---: | --- |
| 普通聊天智能注入 | 0-510 输入 Token | 未命中关键词且基础模块关闭时不增加上下文。 |
...
以下数据使用 `tiktoken` `0.12.0` 的 `cl100k_base` 编码器，于 `2026-07-23` 按当前默认提示词测得。日程样本使用一个简短学生人格、三天历史日程，以及一个包含固定事件、特殊时期和里程碑的当前阶段；历史中最近一天保留完整时间线，较早两天使用摘要；输出样本含 6 个时间线项目、1 个主动窗口和 6 个穿搭单品。它们用于容量规划，不是账单承诺：实际用量会随 Provider 使用的 tokenizer、人格长度、管理员要求、日程内容及模型输出而变化。

### 智能状态注入

智能注入不额外调用 LLM；这些 Token 会作为普通聊天上下文的一部分。基础模块关闭且未命中关键词时增加 `0` Token。下表按开启基础模块计算；命中日程时基础状态不会重复注入。样本基础状态为 `122` Token，全部模块同时命中时为 `508` Token，并受 `smart_context_injection.max_chars` 限制。
...
| 今日日程摘要 | 167 | 45 |
| 当前大时间表摘要 | 195 | 73 |
| 请求完整日程并提示调用 Tool | 203 | 81 |
| 请求完整大时间表并提示调用 Tool | 158 | 36 |
