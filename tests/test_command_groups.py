...
class CommandGroupContractTests(unittest.TestCase):
    ...
    def test_daily_schedule_uses_group_only(self):
        ...
        self.assertIn('@virtual_daily_group.command("要求")', self.source)
        self.assertIn("requirement=GreedyStr", self.source)
        self.assertIn('@filter.llm_tool(name="add_schedule_requirement")', self.source)
        self.assertIn("if not event.is_admin()", self.source)
        self.assertIn("self.storage.active_schedule_requirements", self.source)
        self.assertIn("self.storage.consume_schedule_requirements", self.source)
        self.assertIn("range_days > 366", self.source)
        self.assertIn("len(requirement) > 500", self.source)
        self.assertIn(">= 10", self.source)
        self.assertIn("已有有效日程", self.source)
