...
try:
    ...
    raise SystemExit(
        "缺少预览依赖。请先执行: pip install jinja2 playwright"
    ) from exc
    ...
    return DailyPlan.from_dict(
        {
            ...
            "outfit": {
                ...
                "items": [
                    {"category": "hairstyle", "name": "低马尾", "details": "自然蓬松，搭配棕色发带"},
                    {"category": "underwear", "name": "无痕打底套装", "details": "柔软透气的浅杏色面料"},
                    {"category": "top", "name": "米白针织衫", "details": "细针织圆领，袖口微收"},
                    {"category": "bottom", "name": "焦糖色半身裙", "details": "高腰过膝 A 字剪裁"},
                    {"category": "legwear", "name": "深棕连裤袜", "details": "轻薄哑光质感"},
                    {"category": "outerwear", "name": "格纹短风衣", "details": "适合早晚温差"},
                    {"category": "shoes", "name": "棕色乐福鞋", "details": "低跟软底，适合步行"},
                    {"category": "bag", "name": "复古邮差包", "details": "可放相机和随身物品"},
                    {"category": "accessory", "name": "黄铜叶片胸针", "details": "点缀风衣领口"},
                ],
            },
            "timeline": [
                {"id": "sleep", "start": "00:00", "end": "08:00", "activity": "睡眠", "location": "家", "state": "sleep", "availability": "blocked"},
                {"id": "breakfast", "start": "08:00", "end": "09:30", "activity": "早餐与整理", "location": "家", "state": "available", "availability": "normal"},
                {"id": "museum", "start": "09:30", "end": "12:30", "activity": "参观秋季艺术展", "location": "市美术馆", "state": "focus", "availability": "low"},
                {"id": "lunch", "start": "12:30", "end": "14:00", "activity": "午餐与休息", "location": "河畔餐厅", "state": "social", "availability": "normal"},
                {"id": "walk", "start": "14:00", "end": "17:30", "activity": "城市漫步与拍照", "location": "老城区", "state": "available", "availability": "high"},
                {"id": "dinner", "start": "17:30", "end": "19:30", "activity": "朋友聚餐", "location": "创意园区", "state": "social", "availability": "low"},
                {"id": "reading", "start": "19:30", "end": "22:30", "activity": "整理照片与阅读", "location": "家", "state": "focus", "availability": "normal"},
                {"id": "night", "start": "22:30", "end": "24:00", "activity": "洗漱与入睡", "location": "家", "state": "sleep", "availability": "blocked"},
            ],
            "proactive_windows": [
                {"id": "share-art", "at": "11:30", "intent": "分享展览中喜欢的作品", "audience": "both", "source_item_id": "museum"},
                {"id": "share-photo", "at": "16:00", "intent": "分享老城区的秋日照片", "audience": "private", "source_item_id": "walk"},
            ],
...
def autumn_stage() -> dict[str, Any]:
    return {
        ...
        "weekly_rules": [
            {"weekdays": [1, 3], "start": "09:00", "end": "11:30", "title": "专业课程", "location": "教学楼", "participants": ["同学"], "required": True},
            {"weekdays": [5], "start": "15:00", "end": "17:00", "title": "摄影社活动", "location": "社团活动室", "participants": ["社员"], "required": True},
        ],
        "special_dates": [
            {"date": "2026-10-01", "start": "09:30", "end": "12:30", "title": "国庆艺术展", "location": "市美术馆", "participants": ["朋友"], "required": True}
        ],
        "special_periods": [
            {"name": "国庆假期", "start_date": "2026-10-01", "end_date": "2026-10-07", "constraints": ["暂停常规课程", "安排短途活动"]},
            {"name": "期末复习周", "start_date": "2027-01-10", "end_date": "2027-01-20", "constraints": ["减少娱乐", "优先复习"]},
        ],
        "milestones": [
...
async def render_previews(args: argparse.Namespace) -> list[Path]:
    ...
    selected = list(VIEW_NAMES) if "all" in args.view else list(dict.fromkeys(args.view))
    ...
    with tempfile.TemporaryDirectory(prefix="virtual-life-preview-") as directory:
        ...
        async with PlaywrightHtmlRenderer(temporary_dir, browser_path) as backend:
            ...
            for view in selected:
                if view == "timeline":
                    generated = await renderer.render_timeline(plan, PREVIEW_NOW, long_term_day())
                elif view == "outfit":
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="使用内置示例数据渲染虚拟人生 PNG 预览图。")
    parser.add_argument(
        "--view",
        nargs="+",
        required=True,
    ...
    parser.add_argument("--theme", choices=("dark", "light"), default="dark", help="图片主题。")
    parser.add_argument("--width", type=int, default=1200, help="图片宽度，渲染器会限制在 720-2000。")
    parser.add_argument("--font", default="", help="可选的系统字体名称。")
    parser.add_argument("--browser", help="可选的 Chrome、Edge 或 Chromium 可执行文件路径。")
